...
var CPqDASRClient = function (cfg) {
    ...
    this.isSessionCreated = function () {
        return (serverStatus == SESSION_STATUS.CONNECTED || SESSION_STATUS.IDLE || serverStatus == SESSION_STATUS.LISTENING || serverStatus == SESSION_STATUS.RECOGNIZING)
    }
    ...
    this.isListening = function () {
        ...
        return (serverStatus == SESSION_STATUS.LISTENING || serverStatus == RESULT_STATUS.PROCESSING)
    }
    ...
    this.sendAudioPacket = function (audio, lastPacket) {
        ...
        item += "LastPacket: " + lastPacket + ENTER;
        item += "Content-Length: " + audio.size + ENTER;
    ...
    function cancelRecognition() {
        var item = PRODUCT + " " + VERSION + " " + WS_COMMANDS.CANCEL_RECOGNITION + ENTER;
        cpqdWebSocket.sendMessage(item);
    }

    //libera a sessão no servidor ASR
    function releaseSession() {
        var item = PRODUCT + " " + VERSION + " " + WS_COMMANDS.RELEASE_SESSION + ENTER;
        cpqdWebSocket.sendMessage(item);
    }
    ...
    function identifyResponse(strValue) {
        var arrValues = strValue.split(ENTER);
